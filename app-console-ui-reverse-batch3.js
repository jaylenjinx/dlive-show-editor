...
renderGateConsole=function(root,gate){
  ...
  panel.innerHTML=`<div class="console-subhead"><span>Side Chain</span><span class="confidence verified">CONTROLLED-DIFF VERIFIED</span></div>
    <div class="console-gate-sc-grid">
      ...
      <div class="console-processing-card"><label>Low filter<div class="console-value-row"><input data-k="gate-sc-lo" type="number" min="${GATE_SC_LO_MIN_HZ}" max="${GATE_SC_LO_MAX_HZ}" step="1"><span>Hz</span></div></label><div class="console-readonly-chip">Type raw ${sc?hexByte(sc.loTypeRaw):'—'} · ${sc?.loTypeRaw===0x04?'Lo-Cut':'read only'}</div></div>
      <div class="console-processing-card"><label>High filter<div class="console-value-row"><input data-k="gate-sc-hi" type="number" min="${GATE_SC_HI_MIN_HZ}" max="${GATE_SC_HI_MAX_HZ}" step="1"><span>Hz</span></div></label><div class="console-readonly-chip">Type raw ${sc?hexByte(sc.hiTypeRaw):'—'} · ${sc?.hiTypeRaw===0x03?'Hi-Cut':'read only'}</div></div>
  ...
  for(const [key,kind] of [['gate-sc-lo','lo'],['gate-sc-hi','hi']]){const sel=panel.querySelector(`[data-k="${key}"]`);if(!sc){sel.disabled=true;continue;}const hz=kind==='lo'?sc.loFreqHz:sc.hiFreqHz;sel.value=String(Math.round(hz*100)/100);sel.disabled=!sc.writableShape;sel.onchange=()=>{if(setInputGateSidechainFrequency(gate.channel,kind,sel.value))triggerConsoleRerender();else toast('Gate sidechain frequency write blocked.',true);};}
};
...
function patchManualPeakConsole(shell,comp){
  ...
  if(attack){attack.title=`Manual Peak: exact tested values only (${[...COMP_MP_ATTACK_RAW_TO_MS.values()].map(compFormatTimeMs).join(' or ')})`;attack.value=String(Number((comp.attackMs||0).toPrecision(4)));attack.disabled=!comp.mpTimeWritableShape;attack.onchange=()=>{if(setInputCompressorTime(comp.channel,'attack',attack.value))triggerConsoleRerender();else toast(`Attack write blocked: Manual Peak accepts only ${[...COMP_MP_ATTACK_RAW_TO_MS.values()].map(compFormatTimeMs).join(' or ')}.`,true);};}
  if(release){release.title=`Manual Peak: exact tested values only (${[...COMP_MP_RELEASE_RAW_TO_MS.values()].map(compFormatTimeMs).join(' or ')})`;release.value=String(Number((comp.releaseMs||0).toPrecision(4)));release.disabled=!comp.mpTimeWritableShape;release.onchange=()=>{if(setInputCompressorTime(comp.channel,'release',release.value))triggerConsoleRerender();else toast(`Release write blocked: Manual Peak accepts only ${[...COMP_MP_RELEASE_RAW_TO_MS.values()].map(compFormatTimeMs).join(' or ')}.`,true);};}
