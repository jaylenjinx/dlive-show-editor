...
// Band bytes: [gain_i16_be, frequency_u16_be, width_u16_be, state/type bytes x3].
...
// Canonical high-resolution write: MIDI width index in the high byte, zero fractional byte.
// Existing shows may contain a nonzero fractional low byte; untouched values are preserved exactly.
  ...
  const draw=()=>{
    ...
    for(const b of p.bands){
      ...
      card.innerHTML=`
        ...
        <div class="peq-field readonly"><span>Unmapped state/type bytes</span><code>${hexRange(b.stateBytes)}</code><span class="confidence unknown">READ ONLY</span></div>`;
    ...
    meta.innerHTML=`CH ${p.channel} PEQ frame <code>0x${p.frameStart.toString(16)}</code> · payload ${p.payloadLength} bytes. Untouched low-byte width precision and unknown state bytes are preserved.`;
