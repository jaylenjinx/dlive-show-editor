...
// filter record and the separate sidechain-source record. Every frequency anchor
// matches raw = floor(4608 × log2(hz/4)) within ±1 raw unit (the canonical PEQ
// log-frequency coordinate), so both filters are continuous over their tested
// ranges; filter type/BPF fields are decoded but not written until independent
// one-parameter scenes exist.
const GATE_SC_LO_MIN_HZ=20,GATE_SC_LO_MAX_HZ=5000;
const GATE_SC_HI_MIN_HZ=120,GATE_SC_HI_MAX_HZ=20000;
...
function parseInputGateSidechainFilters(dat){
  ...
  for(let channel=1;channel<=128;channel++){
    ...
    out.push({...r,channel,discriminator:dat[s],
      loFreqOffset:s+3,loFreqRaw:loRaw,loFreqHz:peqFrequencyFromRaw(loRaw),
      loTypeOffset:s+7,loTypeRaw:dat[s+7],
      hiFreqOffset:s+12,hiFreqRaw:hiRaw,hiFreqHz:peqFrequencyFromRaw(hiRaw),
...
function setInputGateSidechainFrequency(channel,kind,hzValue){
  ...
  const range=kind==='lo'?[GATE_SC_LO_MIN_HZ,GATE_SC_LO_MAX_HZ]:kind==='hi'?[GATE_SC_HI_MIN_HZ,GATE_SC_HI_MAX_HZ]:null;if(!range)return false;
  hz=Math.max(range[0],Math.min(range[1],hz));
  ...
  if(kind==='lo'){sc.loFreqRaw=raw;sc.loFreqHz=hz;}else{sc.hiFreqRaw=raw;sc.hiFreqHz=hz;}
...
if(typeof PARAMETER_MAP!=='undefined'){
  const rows=[
    {id:'input-gate-sc-lo-freq',area:'Input gate sidechain',record:'SCF Gate, Input Channel NN',payload:'23-byte state, discriminator 04',field:'SC low-filter frequency',offset:'state +3..4',datatype:'uint16 big-endian log-frequency coordinate',transform:`raw=floor(4608×log2(hz/4)); verified ${GATE_SC_LO_MIN_HZ} Hz…${GATE_SC_LO_MAX_HZ/1000} kHz`,confidence:'verified',write:true,evidence:'CH16 20,100,500 Hz,1k,2k,5k scenes isolate +3..4; every point matches the canonical PEQ log-frequency coordinate exactly.',notes:'Same coordinate proven continuous throughout the format.'},
    {id:'input-gate-sc-hi-freq',area:'Input gate sidechain',record:'SCF Gate, Input Channel NN',payload:'23-byte state, discriminator 04',field:'SC high-filter frequency',offset:'state +12..13',datatype:'uint16 big-endian log-frequency coordinate',transform:`raw=floor(4608×log2(hz/4)); verified ${GATE_SC_HI_MIN_HZ} Hz…${GATE_SC_HI_MAX_HZ/1000} kHz`,confidence:'verified',write:true,evidence:'CH16 120,200,500 Hz,1k,2k,5k,10k,20k scenes isolate +12..13; every point matches the canonical PEQ log-frequency coordinate within ±1 raw unit.',notes:'Same coordinate proven continuous throughout the format.'},
