...
function renderRoutingConsole(root,channel){
  ...
  const shell=document.createElement('section');shell.className='dlive-console-frame routing-console';shell.innerHTML=`
    ...
    </div><div class="console-note">This card keeps its original controlled 208-byte scope. Every send and assign for any mixer config is editable in the Channel state tab's Sends panel, which uses the general Input Mixer layout.</div>`;root.appendChild(shell);
